...
exports.get_current_game = (req, res, next) => {
  const query_string = "SELECT * from game where datediff(now(), game_ending_time_date) > 0;";  
  const payload = [];
  const callback = (err, data) => {
     res.send(data);
  };
...
exports.get_sponsor = (req, res, next) => {
  const query_string = "SELECT * from sponsor where sponsor_id in (SELECT sponsor_id from game_sponsor where game_id = ?);";  
...
exports.get_bracket = (req, res, next) => {
  const query_string = "SELECT * from sport, score where sport_type = ? order by field('elims', 'semi-finals', 'finals'), series;"; 
